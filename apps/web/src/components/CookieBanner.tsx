  ...
  return (
    ...
    >
      <div className="max-w-[1320px] mx-auto px-8 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <p className="text-[12px] text-(--t2) leading-relaxed max-w-[640px]">
          ...
          <Link href="/privacy#cookies" className="text-(--accent) hover:opacity-80 transition-opacity">
            Cookie policy
          </Link>
          .
        </p>
        <div className="flex items-center gap-3 shrink-0">
          <button
            ...
            className="h-8 px-4 rounded-md border border-(--border) text-[12px] text-(--t3) hover:text-(--t2) hover:border-(--t3) transition-colors"
          >
            Reject non-essential
          </button>
          <button
            onClick={() => respond("accepted")}
            className="h-8 px-4 rounded-md bg-(--accent) text-black text-[12px] font-semibold hover:opacity-90 transition-opacity"
          >
            Accept all
          </button>
        </div>
