...
export async function POST(req: Request): Promise<Response> {
  ...
  const result = await container.stripeAdapter.reactivateSubscription(sub.stripeSubscriptionId);

  if (result.isErr()) {
    return Response.json(
      { error: { code: 'STRIPE_ERROR', message: 'Failed to reactivate subscription' } },
