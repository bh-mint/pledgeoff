...
async function getHandler(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await container.interviewGuideRepo.findByIdeaId(ideaId);
  if (existing.isErr()) {
...
async function postHandler(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const planGate = await checkPlanToolGate(userId, 'interview-guide');
  if (!planGate.allowed) {
  ...
  const result = await container.generateInterviewGuideUseCase.execute({
    ideaId,
    userId,
