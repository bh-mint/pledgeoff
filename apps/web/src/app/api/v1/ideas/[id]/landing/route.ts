...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await container._unsafeRepos.landingPageRepo.findByIdeaId(ideaId);
  if (result.isErr()) {
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const decisionResult = await container._unsafeRepos.decisionRepo.findByIdeaId(ideaId);
