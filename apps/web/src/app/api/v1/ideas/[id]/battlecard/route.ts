...
async function getHandler(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await container.battlecardRepo.findByIdeaId(ideaId);
  if (existing.isErr()) {
...
async function postHandler(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const planGate = await checkPlanToolGate(userId, 'battlecard');
  if (!planGate.allowed) {
  ...
  const result = await container.generateBattlecardUseCase.execute({
    ideaId,
    userId,
  ...
  void getTeamSlackWebhook(userId, createSupabaseServiceClient()).then((webhookUrl) => {
    if (webhookUrl) void notifySlack({ webhookUrl, ideaId, ideaText, tool: 'battlecard', traceId });
  });
