...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const userTeamResult = await container.teamRepo.findByMemberId(userId);
  ...
  const authorTeamResult = await container.teamRepo.findByMemberId(idea.userId);
  ...
  const reactionsResult = await container.ideaReactionRepo.findByIdeaIds([ideaId]);
