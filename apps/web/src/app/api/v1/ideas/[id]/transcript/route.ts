...
import { AnalyzeTranscriptRequestSchema } from '@pledgeoff/contracts';
...
async function getHandler(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await container.transcriptAnalysisRepo.findByIdeaId(ideaId);
  if (existing.isErr()) {
...
async function postHandler(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const planGate = await checkPlanToolGate(userId, 'transcript');
  if (!planGate.allowed) {
  ...
  const result = await container.analyzeTranscriptUseCase.execute({
    ideaId,
    userId,
