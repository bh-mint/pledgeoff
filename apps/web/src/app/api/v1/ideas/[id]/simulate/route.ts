...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await container.simulationRepo.findByIdeaId(ideaId);
  if (result.isErr()) {
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const planGate = await checkPlanToolGate(userId, 'rev');
  if (!planGate.allowed) {
  ...
  const decisionResult = await container.decisionRepo.findByIdeaId(ideaId);
