...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await container._unsafeRepos.competitorAnalysisRepo.findByIdeaId(ideaId);
  if (result.isErr()) {
    return Response.json({ error: { code: 'INTERNAL' } }, { status: 500, headers: { 'X-Trace-Id': traceId } });
  }

  return Response.json({ data: result.value ?? null }, { status: 200, headers: { 'X-Trace-Id': traceId } });
}
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await container.analyzeCompetitorsUseCase.execute({
    ideaId,
