...
export default async function DashboardPage() {
  ...
  if (team && isPaidPlan) {
    ...
    const teamIdeasResult = await container.ideaRepo.findByUserIds(allMemberIds);
    if (teamIdeasResult.isOk()) {
      ...
      const [teamDecisions, reactionsResult] = await Promise.all([
        Promise.all(teamIdeas.map((idea) => container.decisionRepo.findByIdeaId(idea.id))),
        container.ideaReactionRepo.findByIdeaIds(teamIdeaIds),
      ]);
