...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: idea ? `Engineering Intelligence · ${idea.text.slice(0, 50)}` : "Engineering Intelligence",
    ...
    <div className="min-h-screen bg-(--canvas)">
      ...
      <div className="max-w-[720px] mx-auto px-4 sm:px-8 py-8 sm:py-12">
        ...
        <div className="flex items-center gap-2 mono text-[11px] mb-8" style={{ color: "var(--t3)" }}>
          ...
          <span>→</span>
          <span style={{ color: "var(--t2)" }}>Build</span>
        </div>
        ...
        <div className="mb-10 pb-8 border-b border-(--border)">
          ...
          <p className="text-[14px] leading-relaxed" style={{ color: "var(--t2)" }}>
            Recommended tech stack, key libraries, and technical gaps — extracted from GitHub engineering signals.
          </p>
          {decision && (
        ...
        <BuildClient ideaId={id} initialAnalysis={analysis} />
      </div>
      <FooterMicro />
    </div>
  );
